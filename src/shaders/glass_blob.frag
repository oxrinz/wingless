...
float fullSDF(vec2 p) {
  // Blob: smin union of growing circles — provides glass texture and shape
  // visuals.
  float d = 1e6;
  for (int i = 0; i < 8; i++) {
    if (scales[i] > 0.001) {
      d = smin(
          d, shapeSDF(p, centers[i], radius * scales[i], widths[i], heights[i]),
          morphK);
    }
  }
  if (maskRadius > 0.0) {
    float mask = shapeSDF(p, maskCenter, maskRadius, maskHalfEx, maskHalfEy);
    // Reveal boundary: morphs from button circles (state=0) to panel mask
    // (state=1). This ties fill directly to openState — state=t means the panel
    // is t-fraction full. The blob circles grow inside this boundary; smin
    // inflation cannot overshoot it.
    float buttons = 1e6;
    for (int i = 0; i < 8; i++) {
      if (scales[i] > 0.001) {
        buttons = smin(buttons,
                       shapeSDF(p, centers[i], radius, widths[i], heights[i]),
                       morphK);
      }
    }
...
void main() {
  ...
  float refractionBand = radius;
  float distFromCenter = 1.0 - clamp(-d / refractionBand, 0.0, 1.0);
  float distortion = 1.0 - sqrt(max(0.0, 1.0 - pow(distFromCenter, 2.0)));

  float eps = 0.5;
  float gx =
      fullSDF(fragCoord + vec2(eps, 0.0)) - fullSDF(fragCoord - vec2(eps, 0.0));
  float gy =
      fullSDF(fragCoord + vec2(0.0, eps)) - fullSDF(fragCoord - vec2(0.0, eps));
  ...
  float internalShadow =
      max(step(d, 0.0), exp(-8.0 * max(d, 0.0) / 400.0)) * interiorDarken;
  vec3 glassColor =
      getBlurredColor(sampleCoord, blurRadius) * (1.0 - internalShadow) * 0.9;
